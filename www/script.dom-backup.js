...
function toggleSavedModal() {
  ...
  if (savedProjectsModal.classList.contains("active")) {
    toggleEditMode(false); // Ensure we start fresh
...
function saveDraft() {
  ...
  const savedLayers = layers.map((layer) => {
    // Map all cells for that layer.
    const cells = layer.div.querySelectorAll(".cell");
    const layerCells = [];
    cells.forEach((c) => {
      layerCells.push({
        color: c.style.backgroundColor,
        percent: c.dataset.percent || "0",
      });
    });
  ...
  // We need to render layers bottom-up
  layers.forEach((layer) => {
    ...
    cells.forEach((cell, i) => {
      ...
      if (
        ...
        const r = Math.floor(i / gridNumber);
        const c = i % gridNumber;
        ctx.fillStyle = bg;
...
function loadDraft(save) {
  try {
    // Confirm check removed to prevent blocking state issues on mobile
    // if (
    ...
    // V2: Multi-layer support
    if (save.version === 2 && save.layers) {
      createGrid(save.gridCount);

      const savedLayers = save.layers;

      // Restore Layer 1
      if (savedLayers.length > 0) {
        if (!layers[0]) {
          throw new Error("Grid creation failed: Layer 0 missing");
        }
        const l1 = layers[0];
        const l1Cells = l1.div.querySelectorAll(".cell");
        const l1Data = savedLayers[0].data;

        l1Data.forEach((d, i) => {
          if (l1Cells[i]) {
            l1Cells[i].style.backgroundColor = d.color;
            l1Cells[i].dataset.percent = d.percent;
          }
        });
        updateLayerPreview(0);
      }

      // Restore subsequent layers
      for (let i = 1; i < savedLayers.length; i++) {
        addLayer(); // Adds to end, becomes active
        const newLayerIndex = layers.length - 1;
        const newLayer = layers[newLayerIndex];
        const layerData = savedLayers[i].data;
        const layerCells = newLayer.div.querySelectorAll(".cell");

        layerData.forEach((d, cellIdx) => {
          if (layerCells[cellIdx]) {
            layerCells[cellIdx].style.backgroundColor = d.color;
            layerCells[cellIdx].dataset.percent = d.percent;
          }
        });
        updateLayerPreview(newLayerIndex);
      }
    } else {
      // Legacy V1 (Single Layer)
      if (!save.data) {
        throw new Error("Legacy save file corrupt: Missing data field");
      }

      createGrid(save.gridCount);
      const l1 = layers[0];
      if (!l1) throw new Error("Grid creation failed");
